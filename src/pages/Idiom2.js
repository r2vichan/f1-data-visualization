...
const controlPanelGroup = svg.append('foreignObject')
  .attr('width', 300)
  .attr('height', 50)
  .attr('x', width - 100 - 250 - margin.right - 50) // Align with the legend
  .attr('y', margin.top - 30) // Position below the legend
   ...
   xAxis
     ...
     .call(
       d3.axisLeft(yScale)
         ...
         .tickFormat((d) => d + maxSum / 2 + 1) // Manipulate the displayed tick values
         .tickPadding(10) // Add padding between the labels and the axis line
     );
